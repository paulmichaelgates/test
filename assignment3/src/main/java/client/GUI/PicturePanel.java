...
public class PicturePanel extends JPanel {
  ...
  //  */
  // public boolean insertImage(File , int row, int col) throws IOException, InvalidCoordinateException {
  ...
  /**
   * Insert an image at position at (col, row)d
   */
  public boolean insertImage(BufferedImage image, int row, int col) throws IOException, InvalidCoordinateException {
    // Check or invalid coordinates
    if (row < 0 || col < 0 || 
        row >= 0 && labels.length <= row || 
        labels[row].length <= col) {
